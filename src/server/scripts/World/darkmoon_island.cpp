...
enum miscRabbit
{
    SPELL_VICIOUS_STREAK = 114085,
    SPELL_HUGE_SHARP_TEETH = 114078,
    SPELL_HUGE_SHARP_TEETH_V = 114080,
    SPELL_LEAP = 134729,

    EVENT_HUGE_SHARP_TEETH = 0,
};

class npc_darkmoon_rabbit : public CreatureScript
{
public:
    npc_darkmoon_rabbit() : CreatureScript("npc_darkmoon_rabbit") { }

    struct npc_darkmoon_rabbitAI : public ScriptedAI
    {
        npc_darkmoon_rabbitAI(Creature* creature) : ScriptedAI(creature) { }

        void Reset() override
        {
            me->RemoveAurasDueToSpell(SPELL_VICIOUS_STREAK);
            _hugeSharpTeethCd = 10 * IN_MILLISECONDS;
        }
        ...
        {
            me->CombatStart(attacker, true);
            me->CastSpell(me, SPELL_VICIOUS_STREAK, true);

            std::list<Player*> playerList;
            GetPlayerListInGrid(playerList, me, 100.f);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_hugeSharpTeethCd <= diff)
            {
                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 10, true))
                {
                    me->CastSpell(target, SPELL_HUGE_SHARP_TEETH_V, true);
                    me->CastSpell(target, SPELL_LEAP, true);
                    me->CastSpell(target, SPELL_HUGE_SHARP_TEETH);
                }
                _hugeSharpTeethCd = 30 * IN_MILLISECONDS;
            }
            else
                _hugeSharpTeethCd -= diff;

            DoMeleeAttackIfReady();
        }

        uint32 _hugeSharpTeethCd;
    };

    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_darkmoon_rabbitAI(creature);
    }
};
...
// Spell Huge Sharp Teeth 114078
class aura_huge_sharp_teeth : public AuraScript
{
    PrepareAuraScript(aura_huge_sharp_teeth);

    void CheckAura(AuraEffect const* aurEff)
    {
        if (!GetCaster() || !GetTarget())
            return;

        if (GetTarget()->GetHealthPct() < 25.f && aurEff->GetTickNumber() >= 3)
        {
            GetTarget()->RemoveAurasDueToSpell(SPELL_HUGE_SHARP_TEETH);
            GetTarget()->RemoveAurasDueToSpell(SPELL_HUGE_SHARP_TEETH_V);
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(aura_huge_sharp_teeth::CheckAura, EFFECT_2, SPELL_AURA_PERIODIC_DAMAGE_PERCENT);
    }
};
...
void AddSC_darkmoon_island()
{
    ...
    new aura_script<aura_huge_sharp_teeth>("aura_huge_sharp_teeth");
}
