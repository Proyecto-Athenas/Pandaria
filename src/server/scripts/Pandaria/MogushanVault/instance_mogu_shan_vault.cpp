            ...
            void OnCreatureCreate(Creature* creature) override
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_MENG:
                        ...
                        if (creature->GetEntry() == NPC_QIANG && SpiritKingsSeq == DONE)
                            events.ScheduleEvent(EVENT_SPIRIT_GUARDS_SAFECHECK, 5 * IN_MILLISECONDS);
                        break;
                    case NPC_ELEGON:
            ...
            void Update(uint32 diff) override
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SPIRIT_GUARDS_SAFECHECK:
                        {
                            for (auto guid : spiritKingsGUIDs)
                            {
                                if (Creature* spiritKing = instance->GetCreature(guid))
                                {
                                    if (spiritKing->GetEntry() == NPC_QIANG)
                                    {
                                        spiritKing->AI()->DoAction(ACTION_FIRST_FIGHT);
                                        spiritKing->AI()->Reset();
                                    }
                                }
                            }
