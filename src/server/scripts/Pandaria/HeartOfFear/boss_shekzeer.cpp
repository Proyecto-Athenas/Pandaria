            ...
            void DoAction(int32 actionId) override 
            {
                switch (actionId)
                {
                    ...
                    case ACTION_ROYAL_GUARDIAN_DIE:
                        if (++guardianDied >= 6 && me->HasAura(SPELL_RETREAT))
...
struct royal_guards_typeAI : public ScriptedAI
{
    ...
    void JustDied(Unit* /*killer*/) override 
    {
        if (Creature* Shekzeer = ObjectAccessor::GetCreature(*me, instance ? instance->GetData64(DATA_SHEKZEER) : 0))
            if (me->GetEntry() == NPC_SETTHIK_WINDBLADE)
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_FIXATE:
                            if (!fixatePresent)
                            {
                                ...
                                // Taunt immune during eff
                                me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, true);
                                me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_ATTACK_ME, true);
                            }
                            events.ScheduleEvent(EVENT_FIXATE, 1 * IN_MILLISECONDS); // Mantain this event up
                            break;
                    }
                }
...
bool npc_setthik_windblade::npc_setthik_windbladeAI::fixatePresent = false;
                ...
                if (Unit* m_caster = GetCaster())
                {
                    if (Unit* m_target = GetHitUnit())
                    {
                        if (m_target->HasAura(SPELL_STICKY_RESIN))
                        {
                            ...
                            {
                                // Transform to Amber Trap if more or equal 5 stucks -- Nefertum: Por algún motivo está contando 1 menos, ahora mismo se deben stackear 6, le bajamos 1 stack para uqe sean 5
                                if (m_aResin->GetStackAmount() >= 4)
                                {
...
class spell_windblade_fixate : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            // Remove tanks
            pList.remove_if(TankSpecTargetSelector());
            pList.remove_if([=](Player* target) { return target && target->HasAura(125390); });
