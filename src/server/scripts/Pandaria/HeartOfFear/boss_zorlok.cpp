...
class boss_zorlok : public CreatureScript
{
    public:
        ...
        struct boss_zorlokAI : public BossAI
        {
            ...
            std::map <uint32, uint32> PlatformDict;
            ...
            void DamageTaken(Unit* attacker, uint32& damage) override
            {
                ...
                // Hotfix was remove
                if (me->HealthBelowPctDamaged(30, damage) && IsHeroic() && !lastEcho)
                {
                    lastEcho = true;
            ...
            void DoAction(int32 actionId) override
            {
                switch (actionId)
                {
                    ...
                    case ACTION_SECOND_PHASE:
                    {
                        ...
                        events.ScheduleEvent(ChooseAction(), urand(20000, 23000));
            ...
            uint32 ChooseAction()
            {
                ...
                auto itr = std::find(choice.begin(), choice.end(), lastEvent);
                if (itr != choice.end()) // first time will be end, so we pick between all the events.
                    choice.erase(itr);
                lastEvent = Trinity::Containers::SelectRandomContainerElement(choice);
                return lastEvent;
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_ATTENUATION:
                        {
                            ...
                            events.ScheduleEvent(action, phase == PHASE_ZORLOK1 ? 40000 : urand(22500, 24500)); // 2s cast + 14,5s cancel + 6-8s cd
                            break;
                        }
                        // Convert platform
                        case EVENT_CONVERT:
                        {
                            ...
                            events.ScheduleEvent(action, phase == PHASE_ZORLOK1 ? 40000 : urand(7000, 9000)); // just 7-9s in second phase.
                        ...
                        case EVENT_CAST_FANDV:
                        {
                            ...
                            events.ScheduleEvent(action, phase == PHASE_ZORLOK1 ? 40000 : urand(18000, 20000)); // 2s cast + 10s duration + 6-8s cd
                            break;
                        }
                        case EVENT_BERSERK:
                        {
                        ...
                        case EVENT_PULL_RAID:
                        {
                            ...
                            if (pos.m_positionZ >= 406.4f)
                            {
                                me->GetMotionMaster()->MoveFall(200);
                            }
                            break;
                        }
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                if (HasAnyBetweenCasterAndTarget()) // no dmg current target if anyone between caster and current target
                    ...
                    if (Aura* inhale = Caster->GetAura(SPELL_INHALE))
                        if (Caster->GetMap())
                            SetHitDamage(Caster->GetMap()->IsHeroic() ? uint32(GetHitDamage() * 2 * (inhale->GetStackAmount() * 0.5f)) : uint32(GetHitDamage() * (inhale->GetStackAmount() * 0.5f)));
            }
...
class spell_convert : public SpellScript
{
    ...
    void SelectTargets(std::list<WorldObject*>& targets)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            pList.remove_if([=](WorldObject* target) { return target && target->ToPlayer() && (!target->ToPlayer()->IsAlive() || target->ToPlayer()->HasAura(SPELL_CONVERT) || target->ToPlayer()->IsGameMaster()); });
            ...
            targets.remove_if([=](WorldObject* target) { return target && target->ToPlayer() && (!target->ToPlayer()->IsAlive() || target->ToPlayer()->HasAura(SPELL_CONVERT) || target->ToPlayer()->IsGameMaster()); });
