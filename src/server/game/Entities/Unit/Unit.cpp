...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (health <= damage)
    {
        ...
        if (victim->ToCreature() && (victim->ToCreature()->GetEntry() == 60776 || victim->ToCreature()->GetEntry() == 63053)) // LITERALMENTE NO SE PUEDE MORIR, SI ALGO LO MATA ES ILEGAL.
        {
            victim->ToCreature()->SetHealth(1);
            damage = 0;
        }
        else
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth)
{
    ...
    if (Player* player = victim->ToPlayer())
    {
        ...
        // Parasitic growth
        if (Aura* aur = player->GetAura(121949))
        {
            if (AuraEffect* aurEff = aur->GetEffect(EFFECT_0))
            {
                aurEff->SetAmount(aurEff->GetAmount() + 0.75f * addhealth);
            }
        }
    }
