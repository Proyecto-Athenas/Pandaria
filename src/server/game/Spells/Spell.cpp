...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            // Mogushan Vaults - Vortex of Energy - Elegon
            if (m_caster->GetMap() && m_caster->GetMapId() == 1008 && m_caster->GetMap()->IsHeroic() && m_caster->GetTypeId() == TYPEID_PLAYER)
            {
                enum { SPELL_TOUCH_OF_THE_TITANS = 130287, NPC_ELEGON = 60410, NPC_ENERGY_CHARGE = 60913 };
                // Check if caster and target have the aura (or not) and target is not Elegon or energy charge, we will catch up that below. 
                if (m_caster->HasAura(SPELL_TOUCH_OF_THE_TITANS) != target->HasAura(SPELL_TOUCH_OF_THE_TITANS) && target->GetEntry() != NPC_ELEGON && target->GetEntry() != NPC_ENERGY_CHARGE)
                    return SPELL_FAILED_VISION_OBSCURED;
                // Here, if caster doesn't have the aura, it shouldn't be able to attack Elegon.
                if (!m_caster->HasAura(SPELL_TOUCH_OF_THE_TITANS) && target->GetEntry() == NPC_ELEGON)
                    return SPELL_FAILED_VISION_OBSCURED;
            }
        }
    }
